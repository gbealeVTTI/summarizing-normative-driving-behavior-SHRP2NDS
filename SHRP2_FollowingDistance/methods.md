...
This application provides an interactive visualization tool to explore driving behavior in the Strategic Highway Research Program Naturalistic Driving Study (SHRP2 NDS). Specifically, the data visualization app shows the distribution of miles driven by following distance values and specific subcategories pertaining to roadway characteristics, as well as driver and vehicle info. Users can filter data based on these factors, which include age, gender, vehicle class, road type, and others. 
...
<figure style="width: 75%; margin: 0 auto;">
  <img src="FollowingDistanceMethodsDocument.svg" 
...
Next, the data is consiladated based on the variables of interest – in this case the following distance bin. The following distance bin, along with the roadway characteristics and miles summation bin were extracted, joined with the associated driver demographics and vehicle info from the unique file ID, and summarized based on miles driven. Once completed, the data no longer contained the unique file ID but did include the driver demographic and vehicle information. The dataset was now expressed at the driver, vehicle, and following distance variable level, as apposed to the file ID level before. This reduces the number of rows considerably and allows the data visualization tool to compare following distance and roadway characteristics with additional filtering available. Table 2 shows the variables and their description at this level.

<h3 style="text-align: center;">Driver, Vehicle, and Following Distance Level Variables</h3>
<table style="width: 100%; border-collapse: collapse;" border="1">
  <caption style="caption-side: top; font-weight: bold;">
    Table 2 – Description of driver, vehicle, and following distance level variables.
  </caption
  <thead style="background-color: #f2f2f2;">
    <tr>
      <th> Variable Name</th>
...
Some final data filtering was required before visualization with this tool. This following distance visualization tool shows following distance values from 0 to 205 meters, excluding the 205 meter endpoint. All other instances of following distance, such as values greater than or equal to 205 meters or those that could not be calculated, have been removed. Thus, an aggregate of 10.07 million miles of driving exist in the SHRP2 NDS data set within those following distance conditions for the visualization tool.

## Step Plot and Box Plot Tabs

The data is shown in two plots in the 'Data Viewer' tab. The first is a step plot which aggregates the miles driven for each following distance bin across all the drivers. This can be viewed either as total miles for each following distance bin or percent of the overall total miles for each following distance bin. The second is a box and whisker plot. This plot shows the box plots for the percentage of total miles of each following distance bin relative to the total miles driven by that unique driver. Here the user can view the mileage breakdown within the population of drivers as opposed to solely relative to the total number of miles driven, which can help visualize the difference between drivers. The download button will download the data used to generate the box plots. This data table icludes mileage accumulations based on the driver ID and the percentage of miles driven relative to the total miles of driven for each driver ID.
